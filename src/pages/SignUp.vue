<template>
  <q-page class="flex flex-center bg-grey-2">
    ...
    >
          ...
          <q-form @submit="submit()">
            ...
            >
              <q-step
                ...
                title="Empresa"
                prefix="1"
                :done="isNotNull(form.company_name) && isNotNull(form.phone)"
                :error="!isNotNull(form.company_name) || !isNotNull(form.phone)"
              >
                <q-card-section class="text-center q-pt-none">
                  <q-icon name="mdi-store-outline" size="xl" color="primary" />
              ...
              <q-step
                ...
                title="Endereço"
                prefix="2"
                :done="!validAddress(form.address)"
                :error="validAddress(form.address)"
              >
                <q-card-section class="text-center q-pt-none">
                  <q-icon
                ...
                >
                  <div class="col-12">
                    <q-input
                      ...
                      @focusout="getAddressToZipCode(form.address.zip_code)"
                      label="Cep"
                      type="tel"
                      mask="#####-###"
                      lazy-rules
      ...
      <div class="col-12 text-center q-pt-sm text-caption">
        developed by Rafael Araujo
      </div>
    </div>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    ...
    validAddress(address) {
      this.isNotNull(address.zip_code);
      this.isNotNull(address.number);
      this.isNotNull(address.complement);
      this.isNotNull(address.public_place);
      this.isNotNull(address.district);
      this.isNotNull(address.city);
      this.isNotNull(address.state);
    },
  },
  computed: {
    maskPhone() {
